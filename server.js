...
async function renderInvoice(tasks, invoiceNo, invoiceDate, paymentStatus, paymentDisplay) {
  ...
  const rows = lines.map((item, index) => `<tr><td style="padding:12px;border:1px solid #cbd5e1;text-align:center;">${index + 1}</td><td style="padding:12px;border:1px solid #cbd5e1;">${item.desc}</td><td style="padding:12px;border:1px solid #cbd5e1;text-align:center;">${item.hsn || ''}</td><td style="padding:12px;border:1px solid #cbd5e1;text-align:right;">${item.amt.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td></tr>`).join('');
